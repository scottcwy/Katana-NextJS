...
export const Showcase = ({ onViewGallery }: ShowcaseProps) => {
  ...
  return (
    <div id="showcase-section" ref={showcaseRef} className="mt-16 mb-20 px-4 showcase-section gallery-section" data-target="gallery-target">
      ...
      >
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-3 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-400">{t('showcase.title')}</h2>
        <p className="text-gray-300/90 max-w-2xl mx-auto text-lg">{t('showcase.description')}</p>
      </motion.div>
        ...
        {PRESET_IMAGES.map((image, index) => {
          ...
          return (
            <motion.div
              ...
              className="group relative overflow-hidden rounded-lg border border-purple-500/20 shadow-lg hover:shadow-purple-500/10 bg-gradient-to-b from-gray-900 to-black transition-all duration-300 hover:scale-[1.02]"
            >
              <div className="absolute inset-0 bg-gradient-to-b from-purple-500/10 to-pink-500/10 mix-blend-overlay pointer-events-none z-10"></div>
              <Image
                src={image.url}
                alt={t(promptKey)}
                width={400}
              ...
              <div className="absolute bottom-0 inset-x-0 p-4 backdrop-blur-sm bg-gradient-to-t from-black/80 via-black/50 to-transparent">
                <div className="text-white">
                  ...
                  <p className="text-sm text-gray-300/90 line-clamp-2">{t(promptKey)}</p>
                </div>
              </div>
            </motion.div>
          );
